...
function getDataAttributes (selection) {
...
var getDataParameters = function (selector, args) {
  if ( selector === void 0 ) selector = 'body';

  var container = d3.select(selector);
  var attrs = getDataAttributes(container);
  var urlVars = camelizeKeys(parse(location.search));
  ...
  var params = _.defaults(args, attrs, urlVars, {
    state: 'CO',
    minYear: 1900,
    maxYear: 2100,
...
function createCutRepeats (cuts, scores) {
  var scoreTests = lodash.map(scores, 'test');
  var scoreTestsUnique = lodash.uniq(scoreTests);

  // no repeats
  if (scoreTests.length === scoreTestsUnique.length) { return lodash.cloneDeep(cuts); }

  var result = [];
  cuts.forEach(function (cut, i) {
    var matches = lodash.filter(scores, {test: cut.test});
    if (!matches.length) {
      result.push(lodash.clone(cut));
    } else {
...
function mergeScores (cuts, scores) {
  var result = [];
  var s = lodash.clone(scores);

  cuts.forEach(function (cut) {
    var match = lodash.find(s, {test: cut.test});

    if (match) {
      lodash.remove(s, match);
      result.push(lodash.merge(lodash.clone(cut), lodash.clone(match)));
    } else {
      result.push(lodash.clone(cut));
    }
  })
...
function removeSkippedYears (cuts, scores) {
  var years = lodash.compact(lodash.map(scores, 'year'));
  var matches = [];
  years.forEach(function (year) {
    var match = lodash.find(cuts, {year: year});
    if (match) { matches.push({year: year, index: cuts.indexOf(match)}); }
  })

  var result = lodash.cloneDeep(cuts);
...
var drawBackground = function (svg, data, x, y, height, isGrowth) {
  ...
  var area = d3.area()
    .x(function (d) { return x(d.data.level); })
